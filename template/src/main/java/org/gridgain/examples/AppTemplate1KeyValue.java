...
public class AppTemplate1KeyValue {
    ...
    private static void updateSingleField(IgniteCache<String, Country> countryCache) {
         ...
         * you read from Ignite caches and attempt to deserialize (see https://issues.apache.org/jira/browse/IGNITE-5038 for details).
         * Update the `CountryEntryProcessor` implementation by using `BinaryObjects` API for records access on the
         * server nodes. Consult with GridGain/Ignite documentation on how to leverage those APIs or check
         * {@see App1KeyValue} application of the `complete` project.
         */
